...
if(options.name && options.pnumber){
    console.log(isInBlackList(options.name, options.pnumber));
}

function readFile(filename = "files/blacklist.txt"){
    var blacklist_data;
    try{
        // console.log("Membaca blacklist yang tersimpan...");
        blacklist_data = fs.readFileSync(filename, 'utf8').split(/\r?\n/);
...
function isInBlackList(uname, uphone){
    try{
